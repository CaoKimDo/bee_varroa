...
extern const uint8_t esp32s3_416_416_bee_espdl[] asm("_binary_esp32s3_416_416_bee_espdl_start");

namespace beedet_detect {
    BeeDet::BeeDet() {
        // Load the model from flash & minimize it
        m_model = new dl::Model((const char *)esp32s3_416_416_bee_espdl, fbs::MODEL_LOCATION_IN_FLASH_RODATA);
...
// Public wrapper
BeeDetDetect::BeeDetDetect() {
    load_model();
}

void BeeDetDetect::load_model() {
    // Assign directly to m_model (DetectWrapper expects this)
    m_model = new beedet_detect::BeeDet();
}
